...
function getIssues(json) {
    ...
    if (json.main !== undefined) {
        ...
        if (!(main instanceof Array)) {
            errors.push('The "main" aggregate has to be either an Array or a String');
        } else {
            var ext2files = {};
            main.forEach(function (filename) {
                ...
                if (/[*]/.test(filename)) {
                    warnings.push('The "main" aggregate cannot contain globs (example: "*.js")');
                }
                if (/[.]min[.][^/]+$/.test(filename)) {
                    warnings.push('The "main" aggregate cannot contain minified files');
                }
                if (isAsset(filename)) {
                    warnings.push('The "main" aggregate cannot contain font, image, audio, or video files');
                }
            ...
            Object.keys(ext2files).forEach(function (ext) {
                var files = ext2files[ext];
                if (files.length > 1) {
                    warnings.push('The "main" aggregate has to contain only 1 file per filetype; found multiple ' + ext + ' files: ' + JSON.stringify(files));
                }
            });
        }
    }
